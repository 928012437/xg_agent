  ...
  <div class="inline-calendar" :class="{'is-weekend-highlight': highlightWeekend}">
    ...
    <table>
      <thead v-show="!hideWeekList">
        <tr>
          <th v-for="(week,index) in weeksList" class="'week is-week-list-'index">{{week}}</th>
        </tr>
      </thead>
      <tbody> 
...
/*class="buildClass(k2, child, formatDate(year, month, child) === value && !child.isLastMonth && !child.isNextMonth)*/
    ...
    render (year, month) {
      let data = getDays({
        ...
        month: month,
        value: this.value,
        rangeBegin: this.convertDate(this.startDate), //开始范围
        rangeEnd: this.convertDate(this.endDate),//结束范围
        returnSixRows: this.returnSixRows, //显示行数
        disablePast: this.disablePast,//过去是否能操作
        disableFuture: this.disableFuture//将来是否能操作
      })
      this.days = data.days
      this.year = data.year
