...
class Goods_XgAgentPage extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('xg_agent_creditshop_goods') . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('xg_agent_creditshop_goods') . ' where 1 and ' . $condition, $params);
		$pager = pagination($total, $pindex, $psize);
		$category = pdo_fetchall('select id,name,thumb from ' . tablename('xg_agent_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post() 
	{
		...
		$id = intval($_GPC['id']);
		$merchid = 0;
		$item = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_creditshop_goods') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if ($item['showlevels'] != '') 
		{
		...
		{
			$stores = pdo_fetchall('select id,storename from ' . tablename('xg_agent_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid']);
		}
...
//		$groups = m('member')->getGroups();
		$category = pdo_fetchall('select id,name,thumb from ' . tablename('xg_agent_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']));
//		if (empty($item['goodstype']) || ($item['goodstype'] == 0))
//		{
//			$goods = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_goods') . ' WHERE id =:id and status=1 and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['goodsid']));
//		}
//		else if ($item['goodstype'] == 1)
//		{
//			$coupon = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_coupon') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['couponid']));
//		}
...
//		$levels = array_merge(array( array('id' => 0, 'key' => 'default', 'levelname' => (empty($_W['shopset']['shop']['levelname']) ? '默认会员' : $_W['shopset']['shop']['levelname'])) ), $levels);
		$allspecs = pdo_fetchall('select * from ' . tablename('xg_agent_creditshop_spec') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
		foreach ($allspecs as &$s ) 
		{
			$s['items'] = pdo_fetchall('select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from ' . tablename('xg_agent_creditshop_spec_item') . ' a left join ' . tablename('xg_agent_virtual_type') . ' b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc', array(':specid' => $s['id']));
		}
		unset($s);
		$html = '';
		$discounts_html = '';
		...
		$options = pdo_fetchall('select * from ' . tablename('xg_agent_creditshop_option') . ' where goodsid=:id order by id asc', array(':id' => $id));
		$specs = array();
		if (0 < count($options)) 
		{
...
//		$dispatch_data = pdo_fetchall('select * from ' . tablename('xg_agent_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				pdo_update('xg_agent_creditshop_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				plog('creditshop.goods.edit', '编辑积分商城商品 ID: ' . $id . ' <br/>商品名称: ' . $data['title']);
			}
			else 
			{
				pdo_insert('xg_agent_creditshop_goods', $data);
				$id = pdo_insertid();
			...
			if ($data['hasoption']) 
			{
				...
				while ($k < $len) 
				{
					...
					{
						pdo_update('xg_agent_creditshop_spec', $a, array('id' => $get_spec_id));
						$spec_id = $get_spec_id;
					}
					else 
					{
						pdo_insert('xg_agent_creditshop_spec', $a);
					...
					while ($n < $itemlen) 
					{
						...
						{
							pdo_update('xg_agent_creditshop_spec_item', $d, array('id' => $get_item_id));
							$item_id = $get_item_id;
						}
						else 
						{
							pdo_insert('xg_agent_creditshop_spec_item', $d);
					...
					if (0 < count($itemids)) 
					{
						pdo_query('delete from ' . tablename('xg_agent_creditshop_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id . ' and id not in (' . implode(',', $itemids) . ')');
					}
					else 
					{
						pdo_query('delete from ' . tablename('xg_agent_creditshop_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id);
					}
					pdo_update('xg_agent_creditshop_spec', array('content' => serialize($itemids)), array('id' => $spec_id));
					$specids[] = $spec_id;
					++$k;
				}
				if (0 < count($specids)) 
				{
					pdo_query('delete from ' . tablename('xg_agent_creditshop_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id . ' and id not in (' . implode(',', $specids) . ')');
				}
				else 
				{
					pdo_query('delete from ' . tablename('xg_agent_creditshop_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id);
				}
				...
				while ($k < $len) 
				{
					...
					{
						pdo_insert('xg_agent_creditshop_option', $a);
						$option_id = pdo_insertid();
					}
					else 
					{
						pdo_update('xg_agent_creditshop_option', $a, array('id' => $get_option_id));
				...
				{
					pdo_query('delete from ' . tablename('xg_agent_creditshop_option') . ' where goodsid=' . $id . ' and id not in ( ' . implode(',', $optionids) . ')');
					$sql = 'update ' . tablename('xg_agent_creditshop_goods') . ' g set' . "\n\t\t\t\t\t" . 'g.mincredit = (select min(credit) from ' . tablename('xg_agent_creditshop_option') . ' where goodsid = ' . $id . '),' . "\n\t\t\t\t\t" . 'g.minmoney = (select min(money) from ' . tablename('xg_agent_creditshop_option') . ' where goodsid = ' . $id . '),' . "\n\t\t\t\t\t" . 'g.maxcredit = (select max(credit) from ' . tablename('xg_agent_creditshop_option') . ' where goodsid = ' . $id . '),' . "\n\t\t\t\t\t" . 'g.maxmoney = (select max(money) from ' . tablename('xg_agent_creditshop_option') . ' where goodsid = ' . $id . ')' . "\n\t\t\t\t\t" . 'where g.id = ' . $id . ' and g.hasoption=1';
					pdo_query($sql);
				}
				else 
				{
					pdo_query('delete from ' . tablename('xg_agent_creditshop_option') . ' where goodsid=' . $id);
					$sql = 'update ' . tablename('xg_agent_creditshop_goods') . ' set minmoney = money,maxmoney = money,mincredit = credit,maxcredit = credit where id = ' . $id . ' and hasoption=0;';
					pdo_query($sql);
				}
				if ($data['hasoption'] !== 0) 
				{
					pdo_update('xg_agent_creditshop_goods', array('total' => $totalstocks), array('id' => $id));
				}
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('xg_agent_creditshop_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('xg_agent_creditshop_goods', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function property() 
	{
		...
		{
			pdo_update('xg_agent_creditshop_goods', array($type => $value), array('id' => $id, 'uniacid' => $_W['uniacid']));
			$statusstr = '';
			if ($type == 'istop') 
			{
				$typestr = '置顶';
