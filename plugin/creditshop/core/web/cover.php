...
class Cover_XgAgentPage extends PluginWebPage 
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'xg_agentv2积分商城入口设置'));
		if (!(empty($rule))) 
		{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($keyword))) 
			{
				if ($keyword['name'] != 'xg_agentv2积分商城入口设置') 
				{
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'xg_agentv2积分商城入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
