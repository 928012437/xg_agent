...
class Comment_XgAgentPage extends PluginWebPage 
{
	...
	protected function getList($checked = 0) 
	{
		...
		$list = pdo_fetchall('SELECT  c.*,g.title,g.thumb FROM ' . tablename('xg_agent_creditshop_comment') . ' c  ' . ' left join ' . tablename('xg_agent_creditshop_goods') . ' g on c.goodsid = g.id  ' . ' WHERE 1 ' . $condition . ' ORDER BY `time` desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('xg_agent_creditshop_comment') . ' c  ' . ' left join ' . tablename('xg_agent_creditshop_goods') . ' g on c.goodsid = g.id  ' . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$item = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_creditshop_comment') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			...
			{
				$goods = pdo_fetch('select id,thumb,title from ' . tablename('xg_agent_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
			}
		}
		if ($_W['ispost']) 
		{
			if (!(empty($item)) && empty($item['virtual'])) 
			{
				...
				pdo_update('xg_agent_creditshop_comment', $arr, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			else 
			{
				...
				else 
				{
					...
					$arr['virtual'] = 1;
					pdo_insert('xg_agent_creditshop_comment', $arr);
					$id = pdo_insertid();
				}
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id FROM ' . tablename('xg_agent_creditshop_comment') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('xg_agent_creditshop_comment', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			$goods = pdo_fetch('select id,thumb,title from ' . tablename('xg_agent_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
