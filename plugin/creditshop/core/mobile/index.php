...
class Index_XgAgentPage extends CreditshopMobilePage 
{
	public function main() 
	{
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('xg_agent_creditshop_adv') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$advs = set_medias($advs, 'thumb');
		$credit = m('member')->getCredit($openid, 'credit1');
		$category = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('xg_agent_creditshop_category') . ' where uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$category = set_medias($category, 'thumb');
		$lotterydraws = pdo_fetchall('select id, title, subtitle, credit, money, thumb,`type`,price from ' . tablename('xg_agent_creditshop_goods') . "\n\t\t\t\t" . 'where uniacid=:uniacid and isrecommand = 1 and `type` = 1 and  status=1 and deleted=0 order by displayorder,id desc limit 4', array(':uniacid' => $uniacid));
