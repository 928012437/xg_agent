...
class Log_XgAgentPage extends CreditshopMobilePage 
{
	...
	public function getlist() 
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('xg_agent_creditshop_log') . ' log' . "\n" . '                left join ' . tablename('xg_agent_creditshop_goods') . ' g on log.goodsid = g.id' . "\n" . '                where 1 ' . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!(empty($total))) 
		{
			$sql = 'SELECT log.id,log.logno,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid,' . 'g.goodstype,log.time_send,log.time_finish,log.iscomment,op.title as optiontitle ' . ' FROM ' . tablename('xg_agent_creditshop_log') . ' log ' . ' left join ' . tablename('xg_agent_creditshop_goods') . ' g on log.goodsid = g.id ' . ' left join ' . tablename('xg_agent_creditshop_option') . ' op on op.id = log.optionid ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function detail() 
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('xg_agent_creditshop_log') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		if (empty($log)) 
		{
			show_json(-1, '兑换记录不存在!');
		}
		...
		{
			$address = pdo_fetch('select * from ' . tablename('xg_agent_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $uniacid, ':openid' => $openid));
		}
		...
		{
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('xg_agent_creditshop_verify') . ' where logid = :id and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':id' => $id, ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			if ($goods['verifytype'] == 0) 
			{
				$verify = pdo_fetch('select isverify from ' . tablename('xg_agent_creditshop_verify') . ' where logid = :id and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':id' => $log['id'], ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			}
			...
			{
				$stores = pdo_fetchall('select * from ' . tablename('xg_agent_store') . ' where  uniacid=:uniacid and status=1 and `type` in (2,3) ', array(':uniacid' => $_W['uniacid']));
			}
			else 
			{
				$stores = pdo_fetchall('select * from ' . tablename('xg_agent_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and `type` in(2,3)', array(':uniacid' => $_W['uniacid']));
			}
			$isverify = pdo_fetch('select * from ' . tablename('xg_agent_creditshop_verify') . ' where logid = ' . $log['id'] . ' and uniacid = ' . $uniacid . ' and isverify = 1 limit 1 ');
	...
	public function express() 
	{
		...
		$log = pdo_fetch('select * from ' . tablename('xg_agent_creditshop_log') . ' where id=:logid and uniacid=:uniacid and openid=:openid limit 1', array(':logid' => $logid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($log)) 
		{
			header('location: ' . mobileUrl('creditshop/log'));
			exit();
		...
		$goods = pdo_fetch('select *  from ' . tablename('xg_agent_creditshop_goods') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':id' => $log['goodsid']));
	...
	public function finish() 
	{
		...
		global $_GPC;
		$logid = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('xg_agent_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (empty($log)) 
		{
			show_json(0, '订单未找到');
		}
		...
		pdo_update('xg_agent_creditshop_log', array('time_finish' => time()), array('id' => $logid, 'uniacid' => $_W['uniacid']));
		show_json(1);
	}
	public function paydispatch() 
	{
		...
		if ($goods['dispatch'] <= 0) 
		{
			pdo_update('xg_agent_creditshop_log', array('dispatchstatus' => 1, 'addressid' => $addressid), array('id' => $log['id']));
		...
		if ($paytype == 'wechat') 
		{
			...
			if (empty($dispatchno)) 
			{
				...
				pdo_update('xg_agent_creditshop_log', array('dispatchno' => $dispatchno, 'addressid' => $addressid), array('id' => $log['id']));
			}
			$params = array();
	...
	public function dispatch_complete() 
	{
		...
		if ($lastlog['dispatchstatus'] < 1) 
		{
			...
			pdo_update('xg_agent_creditshop_log', $record, array('dispatchno' => $tid));
	...
	public function setstore() 
	{
		...
		if (!(empty($upgrade))) 
		{
			pdo_update('xg_agent_creditshop_log', $upgrade, array('id' => $log['id']));
		}
		if (!(empty($upgrade1))) 
		{
			pdo_update('xg_agent_member', $upgrade1, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			if (!(empty($member['uid']))) 
			{
