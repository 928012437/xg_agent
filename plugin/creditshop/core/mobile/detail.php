...
class Detail_XgAgentPage extends CreditshopMobilePage 
{
...
//		}
		...
		if (!(empty($goods))) 
		{
			pdo_update('xg_agent_creditshop_goods', array('views' => $goods['views'] + 1), array('id' => $id));
		...
		$log = array();
		$log = pdo_fetchall('select openid,createtime from ' . tablename('xg_agent_creditshop_log') . "\n" . '                where uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and status > 0 order by createtime desc limit 2 ');
		foreach ($log as $key => $value ) 
		{
			$mem = m('member')->getMember($value['openid']);
			$log[$key]['avatar'] = $mem['headurl'];
			$log[$key]['nickname'] = $mem['nickname'];
			$log[$key]['createtime_str'] = date('Y/m/d H:i', $value['createtime']);
			unset($mem);
		}
		$logtotal = 0;
		$logtotal = pdo_fetchcolumn('select count(1) from ' . tablename('xg_agent_creditshop_log') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and status > 0 ');
		$replys = array();
		$replys = pdo_fetchall('select * from ' . tablename('xg_agent_creditshop_comment') . "\n" . '                where uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and checked = 1 and deleted = 0 order by `time` desc limit 2 ');
		...
		$replytotal = 0;
		$replytotal = pdo_fetchcolumn('select count(1) from ' . tablename('xg_agent_creditshop_comment') . "\n" . '                where uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and checked = 1 and deleted = 0 order by `time` desc ');
	...
	public function getlistlog() 
	{
		...
		$logtotal = 0;
		$logtotal = pdo_fetchcolumn('select count(1) from ' . tablename('xg_agent_creditshop_log') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and status > 0 ');
	...
	public function option() 
	{
		...
		$goods = pdo_fetch('select id,thumb,credit,money,total,title from ' . tablename('xg_agent_creditshop_goods') . ' where id= ' . $id . ' and uniacid = ' . $uniacid . ' ');
		$specs = false;
		$options = false;
		$specs = pdo_fetchall('select * from ' . tablename('xg_agent_creditshop_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
		foreach ($specs as &$spec ) 
		{
			$spec['items'] = pdo_fetchall('select * from ' . tablename('xg_agent_creditshop_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $spec['id']));
		}
		unset($spec);
		$options = pdo_fetchall('select * from ' . tablename('xg_agent_creditshop_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
		if (!(empty($specs))) 
		{
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		{
			$option = pdo_fetch('select total from ' . tablename('xg_agent_creditshop_option') . ' where uniacid = ' . $uniacid . ' and id = ' . $optionid . ' and goodsid = ' . $id . ' ');
			if ($option['total'] <= 0) 
			{
		...
		{
			pdo_delete('xg_agent_creditshop_log', array('goodsid' => $id, 'openid' => $openid, 'status' => 0, 'paystatus' => 0));
			$needpay = true;
			$lastlog = pdo_fetch('select * from ' . tablename('xg_agent_creditshop_log') . ' where goodsid=:goodsid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':goodsid' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
			if (!(empty($lastlog))) 
			{
		...
		{
			pdo_delete('xg_agent_creditshop_log', array('goodsid' => $id, 'openid' => $openid, 'status' => 0));
		}
		$dispatchstatus = 1;
		...
		pdo_insert('xg_agent_creditshop_log', $log);
		$logid = pdo_insertid();
//		if (!(empty($log['realname'])) && !(empty($log['mobile'])))
//		{
//			$up = array('realname' => $log['realname'], 'carrier_mobile' => $log['mobile']);
//			pdo_update('xg_agent_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
//			if (!(empty($member['uid'])))
//			{
...
//		}
		...
		if ($needpay) 
		{
			...
			if ($paytype == 'balance') 
			{
				...
				pdo_update('xg_agent_creditshop_log', array('paytype' => $paystatus), array('id' => $logid));
			}
	...
	public function creditshop_complete() 
	{
		...
		if ($lastlog['status'] < 1) 
		{
			$record = array();
			$record['paystatus'] = '1';
			pdo_update('xg_agent_creditshop_log', $record, array('logno' => $tid));
			$creditlog = pdo_fetch('select id from ' . tablename('xg_agent_creditshop_log') . "\n" . '                    where logno=:logno and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':logno' => $tid, ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
			if (is_h5app()) 
			{
				pdo_update('xg_agent_creditshop_log', array('apppay' => 1), array('logno' => $tid));
			}
		}
		if (is_h5app()) 
		{
	...
	public function lottery() 
	{
		...
		if ((0 < $goods['credit']) && empty($log['creditpay'])) 
		{
			...
			$url=$_W['siteroot'] . 'app/' . substr(mobileUrl('creditshop.index'), 2);
			sendCreditChange($openid,'积分商城抽奖扣除积分','商品名：'.$goods['title'],-$goods['credit'],$url);
			$update['creditpay'] = 1;
			pdo_query('update ' . tablename('xg_agent_creditshop_goods') . ' set joins=joins+1 where id=' . $id);
		}
		$status = 1;
		if ($goods['type'] == 1) 
		{
		...
		pdo_update('xg_agent_creditshop_log', $update, array('id' => $logid));
		if ($status == 2) 
		{
			...
			else if ($goods['goodstype'] == 2) 
			{
				...
				if($credittype=='credit1'){
					$url=$_W['siteroot'] . 'app/' . substr(mobileUrl('creditshop.index'), 2);
					sendCreditChange($openid,'积分商城兑换','商品名：'.$goods['title'],-$goods['credit'],$url);
				}
				...
				pdo_insert('xg_agent_member_log', $data);
				$mlogid = pdo_insertid();
			...
			else if ($goods['goodstype'] == 3) 
			{
				...
				$params = array('openid' => $openid, 'tid' => $log['logno'], 'send_name' => '积分商城红包兑换', 'money' => $money, 'wishing' => '红包领到手抽筋，别人加班你加薪!', 'act_name' => '积分商城红包兑换', 'remark' => '积分商城红包兑换');
			...
			if ($status == 3) 
			{
				pdo_query('update ' . tablename('xg_agent_creditshop_goods') . ' set total=total-1 where id=' . $id);
			}
			...
			{
				pdo_query('update ' . tablename('xg_agent_creditshop_option') . ' set total=total-1 where id=' . $log['optionid']);
			}
		}
