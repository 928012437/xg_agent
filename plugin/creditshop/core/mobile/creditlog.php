...
class Creditlog_XgAgentPage extends CreditshopMobilePage 
{
	...
	public function getlist() 
	{
		...
		if (!(empty($total))) 
		{
			$sql = 'SELECT log.id,log.goodsid,g.title,g.thumb,g.credit,g.type,g.money,log.createtime, log.status, g.thumb FROM ' . tablename('xg_agent_creditshop_log') . ' log ' . ' left join ' . tablename('xg_agent_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
