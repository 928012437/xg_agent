...
class Verify_XgAgentPage extends PluginMobileLoginPage 
{
	...
	public function select() 
	{
		...
		$order = pdo_fetch('select id,verifyinfo from ' . tablename('xg_agent_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		if (empty($order)) 
		{
			show_json(0);
		}
		...
		unset($v);
		pdo_update('xg_agent_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $orderid));
		show_json(1);
	}
	public function check() 
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select id,status,isverify,verifytype,verifynum from ' . tablename('xg_agent_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($order)) 
		{
			show_json(0);
		}
