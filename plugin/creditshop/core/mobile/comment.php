...
class Comment_XgAgentPage extends MobileLoginPage 
{
	...
	public function submit() 
	{
		...
		$uniacid = $_W['uniacid'];
		$logid = intval($_GPC['logid']);
		$log = pdo_fetch('select id,status,iscomment,logno from ' . tablename('xg_agent_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($log)) 
		{
			show_json(0, '兑换记录未找到');
		}
		...
		foreach ($comments as $c ) 
		{
			...
			if (empty($old_c)) 
			{
				...
				pdo_insert('xg_agent_creditshop_comment', $comment);
			}
			else 
			{
				...
				pdo_update('xg_agent_creditshop_comment', $comment, array('uniacid' => $_W['uniacid'], 'goodsid' => $c['goodsid'], 'logid' => $logid));
			}
		}
		if ($log['iscomment'] <= 0) 
		{
		...
		pdo_update('xg_agent_creditshop_log', $d, array('id' => $logid, 'uniacid' => $uniacid));
		show_json(1);
	}
}
?>