...
class Lists_XgAgentPage extends CreditshopMobilePage 
{
	public function main() 
	{
		...
		if (!(empty($cateid))) 
		{
			$cate = pdo_fetch('select id,name from ' . tablename('xg_agent_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cateid, ':uniacid' => $uniacid));
		}
		$category = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('xg_agent_creditshop_category') . "\n\t\t\t\t\t\t" . 'where uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
	...
	public function getlist() 
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('xg_agent_creditshop_goods') . ' where 1 ' . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!(empty($total))) 
		{
			$sql = 'SELECT id,title,thumb,subtitle,`type`,price,credit,money,goodstype FROM ' . tablename('xg_agent_creditshop_goods') . "\n" . '            ' . "\t\t" . 'where 1 ' . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
