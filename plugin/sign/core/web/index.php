...
class Index_XgAgentPage extends PluginWebPage
{
	...
	public function rule()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($set)) {
				...
				pdo_insert('xg_agent_sign_set', $data);
			}
			 else {
				pdo_update('xg_agent_sign_set', $data, array('id' => $set['id'], 'uniacid' => $_W['uniacid']));
			}
	...
	public function records()
	{
		...
		$psize = 20;
		$list = pdo_fetchall('select r.*, m.nickname, m.id as mid, m.headurl from ' . tablename('xg_agent_sign_records') . 'r left join ' . tablename('xg_agent_member') . 'm on r.openid=m.openid ' . $condition . ' order by r.time desc, r.id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('xg_agent_sign_records') . 'r left join ' . tablename('xg_agent_member') . 'm on r.openid=m.openid ' . $condition);
	...
	public function set()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'xg_agentv2:sign') {
					show_json(0, '关键字已存在!');
				}
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'xg_agentv2:sign'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'xg_agentv2:sign', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'xg_agentv2', 'title' => $data['title'], 'description' => $data['desc'], 'thumb' => $data['thumb'], 'url' => mobileUrl('sign'));
