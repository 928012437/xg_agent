...
class Set_XgAgentPage extends PluginWebPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'xg_agentv2签到入口设置'));
			if (!empty($rule)) 
			{
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'xg_agentv2签到入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			if (empty($rule)) 
			{
			...
			$sys = pdo_fetch('select * from ' . tablename('xg_agent_sign_set') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
			if (empty($sys)) 
			{
				$arr['uniacid'] = $_W['uniacid'];
				pdo_insert('xg_agent_sign_set', $arr);
			}
			else 
			{
				pdo_update('xg_agent_sign_set', $arr, array('uniacid' => $_W['uniacid']));
			}
