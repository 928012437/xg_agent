...
class Index_XgAgentPage extends PluginMobileLoginPage
{
	...
	public function dosign()
	{
		...
		pdo_insert('xg_agent_sign_records', $arr);
	...
	public function doreward()
	{
		...
		$record = pdo_fetch('select * from ' . tablename('xg_agent_sign_records') . ' where openid=:openid and `type`=' . $type . ' and `day`=' . $day . ' and uniacid=:uniacid ' . $condition . ' limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (!(empty($record)))
		{
	...
	public function getRecords()
	{
		...
		if (!(empty($total)))
		{
			$sql = 'SELECT * FROM ' . tablename('xg_agent_sign_records') . ' where 1 ' . $condition . ' ORDER BY `time` DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function getRank()
	{
		...
		if (!(empty($type)))
		{
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('xg_agent_sign_user') . ' where 1 ' . $conditioncol;
			$total = pdo_fetchcolumn($sql, $params);
			$list = array();
			if (!(empty($total)))
			{
				$type = 'su.' . $type;
				$sql = 'SELECT su.*, sm.nickname, sm.avatar FROM ' . tablename('xg_agent_sign_user') . ' su left join ' . tablename('xg_agent_member') . ' sm on sm.openid=su.openid where 1 ' . $condition . ' ORDER BY ' . $type . ' DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
				if (!(empty($list)))
				{
					foreach ($list as &$item )
