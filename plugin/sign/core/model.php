...
class SignModel extends PluginModel
{
	...
	{
		global $_W;
		$set = pdo_fetch('select *  from ' . tablename('xg_agent_sign_set') . ' where uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid']));
		if (empty($set)) 
		{
			return '';
		}
	...
	public function getCalendar($year = NULL, $month = NULL, $week = true)
	{
		...
		$records = pdo_fetchall('select *  from ' . tablename('xg_agent_sign_records') . ' where openid=:openid and `type`=0 and `time` between :starttime and :endtime and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':starttime' => $date['firsttime'], ':endtime' => $date['lasttime']));
		if (!(empty($records))) 
		{
	...
	public function getSign($date = NULL) 
	{
		...
		$records = pdo_fetchall('select * from ' . tablename('xg_agent_sign_records') . ' where openid=:openid and `type`=0 and uniacid=:uniacid ' . $condition . ' order by `time` desc ', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		$signed = 0;
		$orderindex = 0;
		$order = array();
		$orderday = 0;
	...
	public function getAdvAward() 
	{
		...
		$records = pdo_fetchall('select * from ' . tablename('xg_agent_sign_records') . ' where openid=:openid and uniacid=:uniacid ' . $condition . ' order by `time` asc ', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (!(empty($records))) 
		{
			foreach ($records as $item ) 
			{
	...
	public function updateSign($signinfo) 
	{
		...
		$info = pdo_fetch('select id  from ' . tablename('xg_agent_sign_user') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		$data = array('openid' => $_W['openid'], 'order' => $signinfo['order'], 'orderday' => $signinfo['orderday'], 'sum' => $signinfo['sum'], 'signdate' => date('Y-m'));
		if (empty($info)) 
		{
			$data['uniacid'] = $_W['uniacid'];
			pdo_insert('xg_agent_sign_user', $data);
			return;
		}
		pdo_update('xg_agent_sign_user', $data, array('id' => $info['id']));
	}
}
?>