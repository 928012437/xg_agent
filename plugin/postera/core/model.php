...
class PosteraModel extends PluginModel 
{
	...
	public function getQR($poster, $member) 
	{
		...
		$qr = pdo_fetch('select * from ' . tablename('xg_agent_postera_qr') . ' where openid=:openid and acid=:acid and posterid=:posterid limit 1', array(':openid' => $member['openid'], ':acid' => $acid, ':posterid' => $poster['id']));
		if (empty($qr)) 
		{
			$qr['current_qrimg'] = '';
			...
			$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $scene_id, 'model' => 0, 'name' => 'XG_AGENT_POSTERA_QRCODE', 'keyword' => 'XG_AGENT_POSTERA', 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
			...
			pdo_insert('xg_agent_postera_qr', $qr);
			$qr['id'] = pdo_insertid();
		}
		else 
		{
			...
			if (($qr['endtime'] < time()) || ($qr['qrtime'] != $poster['timestart'] . $poster['timeend'])) 
			{
				...
				pdo_update('xg_agent_postera_qr', array('ticket' => $ticket, 'qrimg' => $qrimg, 'url' => $result['url'], 'endtime' => $posterendtime, 'qrtime' => $poster['timestart'] . $poster['timeend']), array('id' => $qr['id']));
				$qr['ticket'] = $ticket;
	...
	{
		$font = IA_ROOT . '/addons/xg_agent/static/fonts/msyh.ttf';
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/xg_agent/data/postera/' . $_W['uniacid'] . '/';
		if (!(is_dir($path))) 
		{
			load()->func('file');
			mkdirs($path);
		...
		if (!(empty($qr['goodsid']))) 
		{
			$goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('xg_agent_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['goodsid'], ':uniacid' => $_W['uniacid']));
			if (empty($goods)) 
			{
		...
		if (!(is_file($path . $file)) || ($qr['qrimg'] != $qr['current_qrimg'])) 
		{
			...
			foreach ($data as $d ) 
			{
				...
				if ($d['type'] == 'head') 
				{
					$avatar = preg_replace('/\\/0$/i', '/96', $member['headurl']);
		...
		$img = $_W['siteroot'] . 'addons/xg_agent/data/poster/' . $_W['uniacid'] . '/' . $file;
		if (!($upload)) 
		{
			return $img;
		}
		if (($qr['qrimg'] != $qr['current_qrimg']) || empty($qr['mediaid']) || empty($qr['createtime']) || ((($qr['createtime'] + (3600 * 24 * 3)) - 7200) < time())) 
		{
			...
			$qr['mediaid'] = $mediaid;
			pdo_update('xg_agent_postera_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
		}
	...
	public function getQRByTicket($ticket = '') 
	{
		...
		$qrs = pdo_fetchall('select * from ' . tablename('xg_agent_postera_qr') . ' where ticket=:ticket and acid=:acid limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
		$count = count($qrs);
		if ($count <= 0) 
		{
			return false;
	...
	public function checkMember($openid = '') 
	{
		...
		else 
		{
			...
			$member['headurl'] = $userinfo['headimgurl'];
			$member['province'] = $userinfo['province'];
			$member['city'] = $userinfo['city'];
			pdo_update('xg_agent_member', $member, array('id' => $member['id']));
			$member['isnew'] = false;
		}
		return $member;
	}
