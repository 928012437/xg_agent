...
class Index_XgAgentPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('xg_agent_postera') . ' WHERE 1 ' . $condition . ' ORDER BY isdefault desc,createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);

		foreach ($list as &$row ) {
			$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('xg_agent_postera_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
		}

		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('xg_agent_postera') . ' where 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_postera') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (($item['keyword2'] != $data['keyword2']) && !empty($keyword)) {
				if ($keyword['name'] != 'xg_agent:postera:' . $id) {
					show_json(0, '关键字已存在!');
				}
			}
			if ($plugin_coupon) {
			...
			if ($data['isdefault'] == 1) {
				pdo_update('xg_agent_postera', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
			}


			if (!empty($id)) {
				pdo_update('xg_agent_postera', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('xg_agent_postera', $data);
				$id = pdo_insertid();
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'xg_agent', ':name' => 'xg_agent:postera:' . $id));
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'xg_agent:postera:' . $id, 'module' => 'xg_agent', 'displayorder' => 0, 'status' => $data['status']);
			$keyword_data = array('uniacid' => $_W['uniacid'], 'module' => 'xg_agent', 'content' => trim($data['keyword2']), 'type' => 1, 'displayorder' => 0, 'status' => $data['status']);
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'xg_agent', ':name' => 'xg_agent:postera:auto'));

			if (empty($ruleauto)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'xg_agent:postera:auto', 'module' => 'xg_agent', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'xg_agent', 'content' => 'XG_AGENT_POSTERA', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
		...
		if (!empty($item['goodsid'])) {
			$goods = set_medias(pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('xg_agent_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid'])), 'thumb');
		}
	...
	public function setdefault()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$poster = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_postera') . ' WHERE id = \'' . $id . '\'');
		...
		pdo_update('xg_agent_postera', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
		pdo_update('xg_agent_postera', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT id,title,keyword,keyword2 FROM ' . tablename('xg_agent_postera') . ' WHERE id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);
		...
		foreach ($posters as $poster ) {
			...
			pdo_delete('xg_agent_postera', array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('xg_agent_postera_log', array('posterid' => $poster['id'], 'uniacid' => $_W['uniacid']));
	...
	public function clear()
	{
		...
		global $_GPC;
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/xg_agent/data/postera/' . $_W['uniacid']);
		@rmdirs(IA_ROOT . '/addons/xg_agent/data/qrcode/' . $_W['uniacid']);
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
