...
require XG_AGENT_INC . 'plugin_processor.php';
class PosteraProcessor extends PluginProcessor 
{
	...
	private function responseScan($obj) 
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('xg_agent_postera') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['posterid'], ':uniacid' => $_W['uniacid']));
		if (empty($poster)) 
		{
			return $this->responseDefault($obj);
		}
	...
	private function responseSubscribe($obj) 
	{
		...
		$qrmember = m('member')->getMember($qr['openid']);
		$log = pdo_fetch('select * from ' . tablename('xg_agent_postera_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
		if (empty($log) && ($openid != $qr['openid'])) 
		{
			...
			pdo_insert('xg_agent_postera_log', $log);
			$log['id'] = pdo_insertid();
			...
			if (!(empty($upgrade))) 
			{
				pdo_update('xg_agent_postera_log', $upgrade, array('id' => $log['id']));
			}
		}
					...
					if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) 
					{
						if (!(empty($poster['bedown']))) 
						{
							if (empty($member['agentid'])) 
							{
								if (empty($member['fixagentid'])) 
								{
									...
									if ($author) 
									{
										...
										pdo_update('xg_agent_member', array('agentid' => $qrmember['id'], 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
									}
									else 
									{
										pdo_update('xg_agent_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
									}
							...
							if (!(empty($poster['beagent']))) 
							{
								...
								pdo_update('xg_agent_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => $time), array('id' => $member['id']));
								if ($become_check == 1) 
								{
