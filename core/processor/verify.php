...
class VerifyProcessor extends ComProcessor
{
	...
	public function __construct()
	{
		...
		$this->sessionkey = XG_AGENT_PREFIX . 'order_wechat_verify_info';
		$this->codekey = XG_AGENT_PREFIX . 'order_wechat_verify_code';
	}
			...
			if ($obj->inContext) {
				if (is_numeric($content)) {
					if (8 <= strlen($content)) {
						...
						$orderid = pdo_fetchcolumn('select id from ' . tablename('xg_agent_order') . ' where uniacid=:uniacid and ( verifycode=:verifycode or verifycodes like :verifycodes ) limit 1 ', array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode, ':verifycodes' => '%|' . $verifycode . '|%'));
