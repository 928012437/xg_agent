...
class Index_XgAgentPage extends ComWebPage
{
	...
	public function set()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_sms_set') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($item)) {
				...
				pdo_insert('xg_agent_sms_set', $arr);
				$id = pdo_insertid();
			}
			 else {
				pdo_update('xg_agent_sms_set', $arr, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
