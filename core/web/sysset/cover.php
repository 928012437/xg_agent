...
class Cover_XgAgentPage extends WebPage
{
	protected function _cover($key, $name, $url)
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'xg_agentv2' . $name . '入口设置'));
		$keyword = false;
		$cover = false;
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword1)) {
				if ($keyword1['name'] != ('xg_agentv2' . $name . '入口设置')) {
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'xg_agentv2' . $name . '入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'xg_agentv2', 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => save_media($data['thumb']), 'url' => $url);
