...
$entry = IA_ROOT . '/addons/xg_agentv2/plugin/poster/model.php';
...
if ($op == 'display') {
	...
	$versionfile = IA_ROOT . '/addons/xg_agentv2/version.php';
		...
		if ($ret['result'] == 1) {
			...
			if (!empty($ret['files'])) {
				foreach ($ret['files'] as $file) {
					$entry = IA_ROOT . '/addons/xg_agentv2/' . $file['path'];
	...
	if (!empty($path)) {
		...
		if (is_array($ret)) {
			...
			if (!is_dir(IA_ROOT . '/addons/xg_agentv2/' . $dirpath)) {
				mkdirs(IA_ROOT . '/addons/xg_agentv2/' . $dirpath, '0777');
			}

			$content = base64_decode($ret['content']);
			file_put_contents(IA_ROOT . '/addons/xg_agentv2/' . $path, $content);

			if (isset($ret['path1'])) {
				$path1 = $ret['path1'];
				$dirpath1 = dirname($path1);

				if (!is_dir(IA_ROOT . '/addons/xg_agentv2/' . $dirpath1)) {
					mkdirs(IA_ROOT . '/addons/xg_agentv2/' . $dirpath1, '0777');
				}

				$content1 = base64_decode($ret['content1']);
				file_put_contents(IA_ROOT . '/addons/xg_agentv2/' . $path1, $content1);
			}
		...
		if (!empty($upgrade['upgrade'])) {
			$updatefile = IA_ROOT . '/addons/xg_agentv2/upgrade.php';
		...
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/xg_agentv2/tmp');
		file_put_contents(IA_ROOT . '/addons/xg_agentv2/version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'EWEI_SHOPV2_VERSION\')) {define(\'EWEI_SHOPV2_VERSION\', \'' . $upgrade['version'] . '\');}');
		cache_delete('cloud:modules:upgrade');
		$time = time();
		global $my_scenfiles;
		my_scandir(IA_ROOT . '/addons/xg_agentv2');

		foreach ($my_scenfiles as $file) {
			if (!strexists($file, '/xg_agentv2/data/') && !strexists($file, 'version.php')) {
				@touch($file, $time);
			}
		}
	...
	if ($op == 'checkversion') {
		file_put_contents(IA_ROOT . '/addons/xg_agentv2/version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'EWEI_SHOPV2_VERSION\')) {define(\'EWEI_SHOPV2_VERSION\', \'1.0\');}');
