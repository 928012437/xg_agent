...
class User_XgAgentPage extends WebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT u.*,r.rolename FROM ' . tablename('xg_agent_perm_user') . ' u  ' . ' left join ' . tablename('xg_agent_perm_role') . ' r on u.roleid =r.id  ' . ' WHERE 1 ' . $condition . ' ORDER BY id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('xg_agent_perm_user') . ' u  ' . ' left join ' . tablename('xg_agent_perm_role') . ' r on u.roleid =r.id  ' . ' WHERE 1 ' . $condition . ' ', $params);
		$pager = pagination($total, $pindex, $psize);
		$roles = pdo_fetchall('select id,rolename from ' . tablename('xg_agent_perm_role') . ' where uniacid=:uniacid and deleted=0', array(':uniacid' => $_W['uniacid']));
	...
	protected function post()
	{
		...
		load()->model('user');
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_perm_user') . ' WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (!empty($item)) {
			...
			$role = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_perm_role') . ' WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['roleid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($item['id'])) {
				...
				pdo_update('xg_agent_perm_user', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('xg_agent_perm_user', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,username FROM ' . tablename('xg_agent_perm_user') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('xg_agent_perm_user', array('id' => $item['id']));
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			pdo_update('xg_agent_perm_user', array('status' => $status), array('id' => $item['id']));
