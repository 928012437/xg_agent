...
class Credit_XgAgentPage extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		$params = array(':uniacid' => $_W['uniacid'], ':module' => 'xg_agent', ':credittype' => $type);
		...
		$sql = 'select log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on log.operator<>0 and log.operator<>log.uid and  log.operator=u.uid' . ' left join ' . tablename('xg_agent_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('xg_agent_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('xg_agent_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on log.operator<>0 and log.operator<>log.uid and  log.operator=u.uid' . ' left join ' . tablename('xg_agent_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('xg_agent_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('xg_agent_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ', $params);
