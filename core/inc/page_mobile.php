...
//        header("Access-Control-Allow-Origin：*");
        ...
        if (!$this->is_mobile()) {
            echo json_encode(array('code' => 0, 'message' => '请在手机端打开'));
            die;
        }
    ...
    public function followBar($diypage = false, $merch = false)
    {
        ...
        if ((!($followed) && ($memberid != $mid)) || (!(empty($diyfollowbar['params']['showtype'])) && !(empty($diyfollowbar['params']['isopen'])))) {
            ...
            if (!(empty($mid)) && ($memberid != $mid)) {
                if (!(empty($_SESSION[XG_AGENT_PREFIX . '_shareid'])) && ($_SESSION[XG_AGENT_PREFIX . '_shareid'] == $mid)) {
                    $mid = $_SESSION[XG_AGENT_PREFIX . '_shareid'];
                }
