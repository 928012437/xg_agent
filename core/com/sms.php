...
class Sms_XgAgentComModel extends ComModel
{

    public function temp($mobile, $tplid, $data, $replace = true){
        global $_W;
        $smsset = $this->sms_set();
    ...
    public function send($mobile, $tplid, $data, $replace = true)
    {
        ...
        {
            include_once XG_AGENT_VENDOR . 'dayu/TopSdk.php';
        ...
        {
            include_once XG_AGENT_VENDOR . 'emay/SMSUtil.php';
    ...
    {
        global $_W;
        return pdo_fetch('SELECT * FROM ' . tablename('xg_agent_sms_set') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
    }
    public function sms_temp()
    {
        global $_W;
        $list = pdo_fetchall('SELECT id, `type`, `name` FROM ' . tablename('xg_agent_sms') . ' WHERE status=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
    ...
    public function sms_num($type, $smsset = NULL)
    {
        ...
        if ($type == 'emay')
        {
            ...
            if (!empty($smsset['emay_warn']) && !empty($smsset['emay_mobile']) && ($num < $smsset['emay_warn']) && (($smsset['emay_warn_time'] + (60 * 60 * 24)) < time()))
            {
                ...
                if (empty($emayResult))
                {
                    pdo_update('xg_agent_sms_set', array('emay_warn_time' => time()), array('id' => $smsset['id']));
                }
            }
            return $num;
        }
    ...
    {
        global $_W;
        $template = pdo_fetch('SELECT * FROM ' . tablename('xg_agent_sms') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $tplid, ':uniacid' => $_W['uniacid']));
