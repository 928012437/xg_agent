...
class Common_XgAgentModel
{
    public function getSetData($uniacid = 0)
    {
        ...
        if (empty($set))
        {
            $set = pdo_fetch('select * from ' . tablename('xg_agent_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
            if (empty($set))
            {
                $set = array();
            }
    ...
    public function updateSysset($values, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch('select id, sets from ' . tablename('xg_agent_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($setdata))
        {
            pdo_insert('xg_agent_sysset', array('sets' => iserializer($values), 'uniacid' => $uniacid));
        }
        else
        {
            ...
            pdo_update('xg_agent_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
        }
        $setdata = pdo_fetch('select * from ' . tablename('xg_agent_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
    ...
    public function updatePluginset($values, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch('select id, plugins from ' . tablename('xg_agent_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($setdata))
        {
            pdo_insert('xg_agent_sysset', array('plugins' => iserializer($values), 'uniacid' => $uniacid));
        }
        else
        {
            ...
            pdo_update('xg_agent_sysset', array('plugins' => iserializer($plugins)), array('id' => $setdata['id']));
        }
        $setdata = pdo_fetch('select * from ' . tablename('xg_agent_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        m('cache')->set('sysset', $setdata, $uniacid);
        $this->setGlobalSet();
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
    {
        ...
        if (empty($type))
        {
            $set['notify_url'] = $_W['siteroot'] . 'addons/xg_agent/payment/alipay/notify.php';
    ...
    public function AliPayBarcode($params, $config)
    {
        ...
        $config['notify_url'] = $_W['siteroot'] . 'addons/xg_agent/payment/alipay/notify.php';
    ...
    public function wechat_build($params, $wechat, $type = 0)
    {
        ...
        if ($wechat['version'] == 1)
        {
            ...
            $package['device_info'] = 'xg_agent';
            $package['out_trade_no'] = $params['tid'];
            $package['total_fee'] = $params['fee'] * 100;
            $package['fee_type'] = '1';
            $package['notify_url'] = $_W['siteroot'] . 'addons/xg_agent/payment/wechat/notify.php';
    ...
    public function wechat_child_build($params, $wechat, $type = 0)
    {
        ...
        $package['nonce_str'] = random(32);
        $package['body'] = $params['title'];
        $package['device_info'] = ((isset($params['device_info']) ? 'xg_agent:' . $params['device_info'] : 'xg_agent'));
        ...
        $package['notify_url'] = ((empty($params['notify_url']) ? $_W['siteroot'] . 'addons/xg_agent/payment/wechat/notify.php' : $params['notify_url']));
        $package['trade_type'] = 'JSAPI';
    ...
    public function sendredpack($params, $wechat)
    {
        ...
        if (is_array($wechat['certs']))
        {
            ...
            $certfile = IA_ROOT . '/addons/xg_agent/cert/' . random(128);
            file_put_contents($certfile, $wechat['certs']['cert']);
            $keyfile = IA_ROOT . '/addons/xg_agent/cert/' . random(128);
            file_put_contents($keyfile, $wechat['certs']['key']);
            $rootfile = IA_ROOT . '/addons/xg_agent/cert/' . random(128);
    ...
    public function createNO($table, $field, $prefix)
    {
        ...
        while (1)
        {
            $count = pdo_fetchcolumn('select count(*) from ' . tablename('xg_agent_' . $table) . ' where ' . $field . '=:billno limit 1', array(':billno' => $billno));
            if ($count <= 0)
            {
                break;
            }
    ...
    public function getSec($uniacid = 0)
    {
        ...
        $set = pdo_fetch('select sec from ' . tablename('xg_agent_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($set))
        {
            $set = array();
        }
    ...
    public function paylog($log = '')
    {
        ...
        if (!(empty($openpaylog)))
        {
            $path = IA_ROOT . '/addons/xg_agent/data/paylog/' . $_W['uniacid'] . '/' . date('Ymd');
            if (!(is_dir($path)))
            {
                load()->func('file');
                @mkdirs($path, '0777');
    ...
    {
        $file = IA_ROOT . '/addons/xg_agent/static/js/dist/area/Area.xml';
    ...
    {
        return pdo_fetchall('SELECT  a.uniacid,a.name FROM ' . tablename('xg_agent_sysset') . ' s  ' . ' left join ' . tablename('account_wechats') . ' a on a.uniacid = s.uniacid' . ' left join ' . tablename('account') . ' acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid');
    }
